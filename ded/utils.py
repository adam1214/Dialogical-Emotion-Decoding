...
def emo_trans_prob_TRI_need_softmax_intra(emo_dict, dialogs, add_one_smooth_or_not, val):
    # only estimate anger, happiness, neutral, sadness
    total_transit = 0
...
def emo_trans_prob_TRI_without_softmax_intra(emo_dict, dialogs, add_one_smooth_or_not, val=None):
    # only estimate anger, happiness, neutral, sadness
    a_a = 0
    a_h = 0
    a_n = 0
    ...
    for i in range(len(session)):
        ...
        if Bi_or_Tri == 2:
            ...
            if softmax_or_not == 1:
                ...
                intra_emo_trans_prob_com = emo_trans_prob_TRI_need_softmax_intra(emo_dict, dialogs, 1,val)
                intra_emo_trans_prob_dict[val] = intra_emo_trans_prob_com
            elif softmax_or_not == 0:
                seq_emo_trans_prob_com = emo_trans_prob_TRI_without_softmax_seq(emo_dict, dialogs, 0, val)
                seq_emo_trans_prob_dict[val] = seq_emo_trans_prob_com

                intra_emo_trans_prob_com = emo_trans_prob_TRI_without_softmax_intra(emo_dict, dialogs, 1, val)
                intra_emo_trans_prob_dict[val] = intra_emo_trans_prob_com
...
if __name__ == '__main__':
    ...
    get_val_emo_trans_prob(emo_dict, dialogs_edit, softmax_or_not=0, Bi_or_Tri=3)
    print('+++++++++++++++++++++++++++++++++')
    get_val_emo_trans_prob(C2C_emo_dict, dialogs, softmax_or_not=0, Bi_or_Tri=3)
    print('+++++++++++++++++++++++++++++++++')
